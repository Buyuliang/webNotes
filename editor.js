...
class EditorManager {
    ...
    async loadContent() {
        try {
            ...
            // 临时：先从本地存储加载（测试模式）
            const localContent = localStorage.getItem('webnotes_content');
            if (localContent) {
                this.editor.innerHTML = localContent;
                this.updateStatus('已加载（本地）');
                return;
            }
    ...
    async saveContent() {
        ...
        try {
            ...
            // 临时：保存到本地存储（测试模式）
            try {
                localStorage.setItem('webnotes_content', content);
                localStorage.setItem('webnotes_saved', new Date().toISOString());
                
                this.updateStatus('已保存（本地）');
                this.lastSaved.textContent = `最后保存: ${new Date().toLocaleTimeString()}（本地存储）`;
    ...
    formatSelection(command, value = null) {
        ...
        this.editor.focus();
        this.updateStatus('已格式化');
        setTimeout(() => this.updateStatus('就绪'), 1000);
    }
}
...
function formatText(command) {
    if (window.editorManager) {
        window.editorManager.formatSelection(command);
    } else {
