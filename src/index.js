...
async function handleAuth(request, env, path, corsHeaders) {
  ...
  if (path === '/auth/callback') {
    ...
    if (!code || !state) {
      return Response.redirect(`${url.origin}/?error=invalid_callback`);
    }
    ...
    if (tokenData.error) {
      return Response.redirect(`${url.origin}/?error=token_error`);
    }
    ...
    // 设置 cookie 并重定向
    const redirectUrl = `${url.origin}/?session=${sessionId}`;
